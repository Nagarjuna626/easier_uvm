...
sub parse_cmdline {
    ...
    foreach $argnum ( 0 .. $#ARGV) {
        ...
        if ( $argnum != $pnum_c && $argnum != $pnum_r &&
...
sub gen_bfm {
    ...
    print FH "\n";
    print FH "interface ${agent_name}_bfm(${agent_if} if_port); \n";
    print FH "\n";
    print FH "  timeunit      $timeunit;\n";
    print FH "  timeprecision $timeprecision;\n";
    print FH "\n";
...
sub gen_monitor {
    ...
    {
        print FH "\n";
        print FH "  // Methods build_phase, run_phase, and do_mon generated by setting monitor_inc in file $tpl_fname{$agent_name}\n";
...
sub gen_agent {
    ...
    unless ( exists $agent_generate_methods_after_class{$agent_name} && $agent_generate_methods_after_class{$agent_name} eq "NO" )
    {
        ...
        print FH "    `uvm_fatal(get_type_name(), \"${agent_name} virtual interface is not set!\")\n";
        print FH "\n";
...
sub gen_env_seq_lib {
    ...
    for ( my $i = 0 ; $i < $number_of_instances{$agent_name} ; $i++ ) {
        ...
        print FH "      seq.start(m_env.m_${agent_name}${suffix}_agent.m_sequencer, this);\n";
        print FH "    end\n";
    ...
    foreach my $extra_agent ( @{ $env_agents{"${agent_name}_env"} } ) {
        ...
        print FH "        `uvm_error(get_type_name(), \"Failed to randomize sequence\")\n";
        print FH "      seq.set_starting_phase( get_starting_phase() );\n";
        print FH "      seq.start(m_env.m_${extra_agent}_agent.m_sequencer, this);\n";
        print FH "    end\n";
    }
    print FH "  join\n";
    print FH "\n";
...
sub gen_dut_inst() {
  ...
  XPROC_INTF: while ($line) {
        if ($if_name) {
            unless ( exists $if_instance_names{$if_name} ) {
                warning_prompt("INTERFACE $if_name GIVEN IN FILE $dut_pfile DOES NOT EXIST");
            }
        ...
        while ($line) {
            if ( $line =~ m/\s*#/ ) {    #script comments - ignore
                ...
                if ($if_name) {
                    push @port_list1, "    .$1";
                    push @port_list2, "($if_instance_names{$if_name}.$2),";
                }
                else {
                    push @port_list1, "    .$1";
                    push @port_list2, "($2),";
    ...
    print FH "${dut_iname} (\n";
    if (@port_list2) { chop($port_list2[-1]); }    #remove trailing ','
...
sub gen_top_seq_lib {
    ...
    foreach my $env (@reg_env) {
        align("  m_${env}_seq ", "= ${env}_default_seq", "::type_id::create(\"m_${env}_seq\");");
    }
    ...
    if ($regmodel) {
        foreach my $env (@reg_env) {
            print FH "    if ( !m_${env}_seq.randomize() )\n";
            print FH "      `uvm_error(get_type_name(), \"Failed to randomize sequence\")\n";
            if ( $env =~ /(\w+)_env/ ) {
                ...
                for ( my $i = 0 ; $i < $number_of_instances{$aname} ; $i++ ) {
                    ...
                    if ( ${reg_access_instance{$aname}} ne "" ) {
                        print FH "    m_${env}_seq.regmodel${suffix} = regmodel${reg_access_instance{$aname}}${suffix};\n";
                    }
                    else {
                        # If instance = "", use the top-level regmodel
                        print FH "    m_${env}_seq.regmodel${suffix} = regmodel;\n";
                    }
               }
            }
        }
    ...
    foreach my $aname (@top_env_agents) {
        ...
        for ( my $i = 0 ; $i < $number_of_instances{$aname} ; $i++ ) {
            ...
            print FH "          `uvm_error(get_type_name(), \"Failed to randomize sequence\")\n";
            print FH "        seq.set_starting_phase( get_starting_phase() );\n";
            print FH "        seq.start(m_${aname}${suffix}_agent.m_sequencer, this);\n";
            print FH "      end\n";
...
sub gen_top() {
    ...
    foreach my $intf (@agent_list) {
        for ( my $i = 0 ; $i < $number_of_instances{$intf} ; $i++ ) {    
            my $suffix = "_${i}";                                        
            align("  ${intf}_if  ", "${intf}_if${suffix} ();", "");      
        }                                                                
    }
    align("\n", "", "");
    ...
    if ( $split_transactors eq "YES" ) {
        ...
        foreach my $intf (@agent_list) {
            for ( my $i = 0 ; $i < $number_of_instances{$intf} ; $i++ ) {                       
                my $suffix = "_${i}";                                                           
                align("  ${intf}_bfm  ", "${intf}_bfm${suffix} (${intf}_if${suffix});", "");    
            }                                                                                   
        }
        align("\n", "", "");
    }
...
sub gen_riviera_script {
    ...
    print FH "alib work\n\n";
    print FH "#compile the dut code\n";
    print FH "set cmd \"alog -uvm -f ../${dut_tb_dir}/files.f\"\n";
    print FH "eval \$cmd\n\n";

    if($common_pkg_fname) {
        open (FFH, "<", "${dut_tb_path}/files.f") || die "Exiting due to Error: can't open file: ${dut_tb_path}/files.f";
        do {
            print FH "set cmd \"alog -uvm  ../${dut_tb_dir}/${common_pkg_fname}\"\n";
            print FH "eval \$cmd\n\n";
        } unless (grep /$common_pkg_fname/,<FFH>);
         close FFH;
    }
    ...
    do {
        print FH "#compile the register model package\n";
        print FH "set cmd \"alog -uvm  ../tb/regmodel/${regmodel_file}\"\n";
        print FH "eval \$cmd\n\n";
    } if $regmodel;
    ...
    if($common_env_pkg_fname) {
        open (FFH, "<", "${dut_tb_path}/files.f") || die "Exiting due to Error: can't open file: ${dut_tb_path}/files.f";
        do {
            print FH "set cmd \"alog -uvm " . $incdir . " ../tb/include/${common_env_pkg_fname}\"\n";
            print FH "eval \$cmd\n\n";
        } unless (grep /$common_env_pkg_fname/,<FFH>);
         close FFH;
    }
    ...
    print FH "foreach  ele \$agent_list {\n";
    print FH "  if {\$ele != \" \"} {\n";
    print FH "    set cmd  \"alog -uvm " . $incdir . "+incdir+../tb/\"\n";
    print FH "    append cmd \$ele \"/sv ../tb/\" \$ele \"/sv/\" \$ele \"_pkg.sv\ ../tb/\" \$ele \"/sv/\" \$ele \"_if.sv\"\n";
    if ( $split_transactors eq "YES") {
        print FH "    append cmd \" ../tb/\" \$ele \"/sv/\" \$ele \"_bfm.sv\"\n";
    }
    ...
    if ( defined $syosil_scoreboard_src_path ) {
        print FH "set cmd  \"alog -uvm +incdir+../../$syosil_scoreboard_src_path ../../$syosil_scoreboard_src_path/pk_syoscb.sv\"\n";
        print FH "eval \$cmd\n\n";
    }
    ...
    print FH "set cmd  \"alog -uvm -timescale $timeunit/$timeprecision "
      . $incdir
      . "+incdir+../tb/\"\n";
    print FH "append cmd \$tb_name \"_tb/sv ../tb/\" \$tb_name \"_tb/sv/\" \$tb_name \"_th.sv\"\n";
    print FH "eval \$cmd\n\n";
